...
def start_gateway_registration(*, endpoint: str, logger, stream: str = "") -> None:
    """
    ...
    def _send_register():
        ...
        try:
            ...
            sock.close()
            logger.info("Sent %s to %s:%d endpoint=%s", intent_type, gw_ip, gw_port, endpoint)
        except OSError as exc:
            logger.warning("Failed to send %s: %s", intent_type, exc)
